...
public int binarySearch(int catNumToFind)
{
  ...
  while(low <= high) {
    ...
    if(store[checkIndex].getCatNum() == catNumToFind) {
      return store[checkIndex].getInventory();
    } else if(store[checkIndex].getCatNum() < catNumToFind) {
      low = checkIndex+1;
    } else {
      high = checkIndex-1;
    }
